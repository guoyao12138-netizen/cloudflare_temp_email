...
- feat: |Android| InkBlue Writer v0.8.0 (Android only): add Gemini protocol support (Google official / relay); add Tavily search API key so web search across AI writing and the agent works on every protocol (incl. Gemini / OpenAI-compatible), falling back to Anthropic/Gemini native search when unset; per-service reasoning-effort selector (off/low/medium/high, mapped to Claude effort, OpenAI reasoning_effort, Gemini thinkingBudget)
- feat: |Android| InkBlue Writer v0.7.0: add a per-book writing-assistant agent (multi-turn chat grounded in the book's worldbuilding/outline/chapters, producing usable prose and revisions); add a writing-skill system (5 built-in technique presets, user-editable, injected into AI prompts); add web search to AI writing and the agent (Anthropic server-side web_search with pause_turn resume); move bookshelf/chapter action buttons into bottom-centered pill bars
- feat: |Android| InkBlue Writer v0.6.1: UI polish — all AI entry points moved to the lower-middle of the screen (editor symbol bar leads with an AI button; lore/outline pages use bottom-centered dual pills; lore revision bottom-centered); add a macOS Intel desktop edition (Compose Desktop DMG published with each release) sharing the bookshelf/chapters/outline/worldbuilding/style-library/multi-AI features, storing data in ~/.inkblue-writer
- feat: |Android| InkBlue Writer v0.6.0: add outline planning (plot-stage cards with reordering, one-click AI planning that extends existing outlines, and outline-aware continuation) and a style-imitation agent — import/paste novel text (UTF-8/GBK auto-detected) to distill a seven-part style guide into a style library, then continue or rewrite selections in that style from the editor; Room migration v2→v3
- feat: |Android| InkBlue Writer v0.5.0: worldbuilding generation refined into 8 standalone modules (framework / power system / region / faction / single character / item / starter village / opening cast); per-entry AI revision (expand / condense / consistency audit / custom instruction); AI config upgraded to multi-service profiles with relay & API-proxy support, plus writer+reviewer role assignment for multi-AI collaborative generation (draft → review → final)
- feat: |Android| InkBlue Writer v0.4.0: screen adaptation (adaptive multi-column grids on tablets/landscape, editor capped at a readable width, rotation no longer recreates the activity); frame-rate features (high-refresh-rate toggle + live FPS overlay); fix APK signature conflicts on upgrade by signing every build with a fixed in-repo keystore (upgrading from ≤v0.3.0 requires one uninstall/reinstall)
- feat: |Android| InkBlue Writer v0.3.0: add one-click AI worldbuilding — world framework (5-8 self-consistent entries from a world-type choice plus author notes), cultivation/power system (generated from a chosen "realm expressiveness" style), starter village (grounded in existing lore), and opening-cast generation (characters with personality, appearance, backstory, and a unified naming style); results are previewed and batch-saved on confirmation
