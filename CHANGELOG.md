...
- feat: |Android| 墨蓝写作 v0.5.0：世界观生成细化为 8 个独立模块（框架/修炼体系/地区/势力/单个角色/物品法宝/新手村/出场角色）；新增单条设定的 AI 修订（扩写/精简/一致性校对/自定义指令）；AI 配置升级为多服务管理，支持中转站与 API 代理接入，并可指定主笔+审校模型实现多 AI 协作生成（起草→审校定稿）
- feat: |Android| 墨蓝写作 v0.4.0：屏幕适应（平板/横屏自适应多列网格、编辑器限宽居中、旋转不丢编辑状态）；帧数功能（高刷新率开关 + 实时 FPS 显示）；修复安装新版签名冲突——改用仓库内固定密钥统一签名，此后版本可直接覆盖升级（从 ≤v0.3.0 升级需卸载重装一次）
- feat: |Android| 墨蓝写作 v0.3.0：新增 AI 一键构筑世界观——世界观框架（按世界类型+作者补充生成 5-8 条自洽设定）、修炼体系（按境界表现力生成境界划分与突破方式）、新手村（基于已有设定生成开局之地）、出场角色（批量生成含性格/外貌/背景/统一命名风格的开篇角色）；生成结果预览确认后批量入库
